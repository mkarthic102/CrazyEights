...
public class Game {
   ...
   private Player nextPlayer() {
      ...
      /* determines the next player if the first card
         on the discard pile is a Queen */
      if (current == -1) {
      ...
      // determining the next player (current) if a player is skipped
      else if (discardPile.top().getRank() == 12) {
         current = current + 2;
         if (current >= players.length) {
            current = current - players.length;
         }
      ...
      // determining the next player (current) if a player isn't skipped
      else {
         current++;
         if (current == players.length) {
            current = 0;
      ...
      // outputting the skipped player based on the index of the current player
      if (current == 0) {
         displaySkippedPlayer(players[players.length - 1].getName());
         waitForUser();
         displayGameState();
         return players[current];
      }
      else {
         displaySkippedPlayer(players[current - 1].getName());
         waitForUser();
         displayGameState();
         return players[current];
      }
